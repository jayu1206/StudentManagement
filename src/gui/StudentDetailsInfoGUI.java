	...
	StudentDetailsInfoGUI(StudentBean bean, String classId, String className){
		...
		// step 3 : creating JLabel for Heading
				...
				/* P1 for first tab data  */
				 JPanel p1=createContactPanel1(this.bean);   // Call method for set the 1st tab frame contenct
			   ...
			   /* heading_lbl=new JLabel("Select record to edit");
				...
				heading_lbl.setForeground(Color.red);
				heading_lbl.setFont(f);
				heading_lbl.setFont(f);
				add(heading_lbl);
			    */
      ...
      // double click on table row and open other window code
		...
		*/
         ...
         for(StudentDecoding decoBean : studBean.getListDecoding()){
        	 ...
        	 model.addRow(new Object[]{decoBean.getDecoId(),decoBean.getWeek(),decoBean.getDate(),decoBean.getBook(),decoBean.getLesson()
        			 ,decoBean.getForm(),decoBean.getScore()});
	...
	modelRate = new DefaultTableModel();
     ...
     JTableHeader header= jtRate.getTableHeader();
       header.setBackground(Color.yellow);
        ...
        JScrollPane scroller = new JScrollPane(jtRate, 
                 JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
...
if(e.getSource()==btnSaveRate){
			...
			for(int i =0 ; i<row ; i++){
				if(jtRate.getModel().getValueAt(i, 1).toString().length()==0){
			        ...
			        if(!mtch.matches()){
			        	JOptionPane.showMessageDialog(this,"Date Date Format invalid");
			        	flag=false;
			        }
	...
	/*public static void main(String args[]){
		new StudentDetailsInfoGUI(null,null,null);
